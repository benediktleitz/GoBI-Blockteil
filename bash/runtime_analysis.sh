...
echo -e "k\toffset\tthreshold\tpair_mode\tmode\tsample\telapsed\tuser_sec\tsystem_sec\tmax_rss_kb" > "$TIME_SUMMARY"
...
write_time_summary() {
  ...
  echo -e "${k}\t${offset}\t${threshold}\t${pair_mode}\t${mode}\t${sample}\t${elapsed:-NA}\t${user_sec:-NA}\t${system_sec:-NA}\t${max_rss:-NA}" >> "$TIME_SUMMARY"
}
...
run_one() {
  ...
  write_time_summary "$time_file" "dna" "$sample"
}

for sample in "${SAMPLES[@]}"; do

  r1=$(ls "$FASTQ_BASE"/${sample}-*_R1_001.fastq.gz | head -n1)
  ...
  for k in "${K_VALUES[@]}"; do
    ...
    for pair_mode in "${PAIR_MODES[@]}"; do
      for r in $(seq 1 "$RUNS_PER_SETTING"); do
        ...
        echo "[$run_idx/$total_runs] EXECUTE k=$k offset=$offset threshold=$t pair_mode=$pair_mode"
        run_one "$k" "$offset" "$t" "$pair_mode" "$out_dir"
  ...
  for o in "${OFFSET_VALUES[@]}"; do
    ...
    for pair_mode in "${PAIR_MODES[@]}"; do
      for r in $(seq 1 "$RUNS_PER_SETTING"); do
        ...
        run_one "$k" "$o" "$t" "$pair_mode" "$out_dir"
        executed_runs=$((executed_runs + 1))
      done
    done
  done
